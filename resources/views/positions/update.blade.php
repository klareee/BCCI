<x-app-layout>
    ...
    <form action="{{ route('positions.update', ['position' => $position->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to update this record?')">
        @method('PUT')
        @csrf
        <div class="max-w-xl mx-auto p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <header class="pb-3">
                <h2 class="text-lg font-medium text-gray-900">{{ __('Position Information') }}</h2>
            </header>
            ...
            <div class="flex justify-end gap-2">
                ...
                <a href="{{ route('positions.index') }}" class="flex items-center gap-1 px-4 py-2 text-sm font-medium text-white transition-colors duration-150 border border-transparent rounded-lg bg-red-500 hover:bg-red-600 active:bg-red-600 focus:outline-none focus:ring focus:ring-red-200">
                    Cancel
                </a>
            </div>
        </div>
