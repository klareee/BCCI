<x-app-layout>
    <x-slot name="header">
        {{ __('Submit Overtime Hours') }}
    </x-slot>

    <form action="{{ route('overtimes.store') }}" method="POST" onsubmit="return confirm('Are you sure you want to create this record?')">
        @csrf
        <div class="max-w-xl mx-auto p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            ...
            <div class="mb-4">
                ...
                <select
                    ...
                    @foreach ($entries as $entry)
                        <option
                            value="{{ $entry->id }}"
                            @if ($entry->id == old('entry_id')) selected @endif
            ...
            <div class="mb-4">
                ...
                <textarea
                    ...
                    id="purpose"
                    rows="5"
                    class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">{{ old('purpose') }}</textarea>
